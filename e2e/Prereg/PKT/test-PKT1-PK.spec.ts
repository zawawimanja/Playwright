...
test('Prereg PK PKT', async ({ page }) => {
  ...
  await page2
    .getByRole('textbox', { name: 'Identification No.*' })
    .fill('680821045674');
  ...
  await preferredSOCSOOfficePage.preferredSOCSOOfficeButton.waitFor({
    state: 'visible',
  });
  ...
  await expect(
    page1.getByRole('button', { name: 'Pull Dependent' }),
  ).toBeVisible();
  ...
  const schemeRefValue = await schemeRefNoTextbox.inputValue();
