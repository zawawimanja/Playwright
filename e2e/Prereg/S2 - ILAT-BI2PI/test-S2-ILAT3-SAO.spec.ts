...
test('Prereg SAO ILAT S2', async ({ page }) => {
  ...
  await page2
    .locator('[id^="Contribution56_EmployerCode-"]')
    .nth(0)
    .fill('B3400033128Y');
  ...
  for (let i = 0; i < 180; i++) {
    ...
    await page2
      .locator("input.textInput[id^='Contribution56_WagesInfo_Year-']")
      .nth(i)
      .fill('2012');

    // Select the month option
    await page2
      .locator("[id^='Contribution56_WagesInfo_Month-']")
      .nth(i)
      .selectOption('2');

    // Fill the wages input
    await page2
      .locator("input.textInput[id^='Contribution56_WagesInfo_Wages-']")
      .nth(i)
      .fill('100');
    // Select an option from the combobox (3rd combobox)

    await page2
      .locator("[id^='Contribution56_WagesInfo_Source-']")
      .nth(i)
      .selectOption('9607901');

    //Check the checkbox with the label "Recommend"
    //  await page2.getByRole("checkbox", { name: "Recommend", exact: true }).nth(i).check();

    await page2
      .locator("[id^='Contribution56_WagesInfo_Recommend-']")
      .getByLabel('Recommend')
      .nth(i)
      .check();
  }
